            ...
            <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $product->image ? 'Заменить основное изображение' : 'Загрузить основное изображение *' }}
                    </label>
                ...
                @if($product->gallery && count($product->gallery) > 0)
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Текущая галерея</label>
                        <div class="grid grid-cols-4 gap-4">
                            @foreach($product->gallery_urls as $index => $imageUrl)
                                <div class="relative group">
                                    <img src="{{ $imageUrl }}" alt="Gallery image" class="w-full h-24 object-cover rounded-lg">
                                    <form action="{{ route('admin.products.gallery.remove', $product) }}" method="POST" class="absolute top-1 right-1">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="image_index" value="{{ $index }}">
                                        <button type="submit" class="bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
                                                onclick="return confirm('Удалить это изображение?')" title="Удалить изображение">×</button>
                                    </form>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Добавить изображения в галерею</label>
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-gray-400 transition-colors">
                        <div class="space-y-1 text-center">
                            <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="gallery" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                    <span>Загрузить файлы</span>
                                    <input id="gallery" name="gallery[]" type="file" accept="image/jpeg,image/png,image/jpg,image/gif" multiple class="sr-only" onchange="previewGallery(this)">
                                </label>
                                <p class="pl-1">или перетащите сюда</p>
                            </div>
                            <p class="text-xs text-gray-500">Можно выбрать несколько файлов. PNG, JPG, GIF до 2MB каждый</p>
                        </div>
                    </div>
                    <div id="galleryPreview" class="mt-4 grid grid-cols-4 gap-4 hidden"></div>
                    @error('gallery.*')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
    ...
    if (files.length > 0) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            reader.onload = function(e) {
                ...
                div.innerHTML = `<img class="h-24 w-full object-cover rounded-lg" src="${e.target.result}" alt="Gallery preview">`;
